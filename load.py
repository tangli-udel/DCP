...
from torchvision.datasets import ImageNet, ImageFolder, CIFAR100, CIFAR10
from imagenetv2_pytorch import ImageNetV2Dataset as ImageNetV2
from datasets import _transform, CUBDataset, _transform_origin, RemappedImageFolder, CocoCaptionsDataset, Flickr30kCaptionsDataset
...
IMAGENETA_DIR = '' # REPLACE THIS WITH YOUR OWN PATH
CUB_DIR = '' # REPLACE THIS WITH YOUR OWN PATH
CALTECH101_DIR = '' # REPLACE THIS WITH YOUR OWN PATH
EUROSAT_DIR = "" # REPLACE THIS WITH YOUR OWN PATH
OXFORDPET_DIR = "" # REPLACE THIS WITH YOUR OWN PATH
FOOD101_DIR = "" # REPLACE THIS WITH YOUR OWN PATH
MSCOCO_DIR = ""
SUN397_DIR = ""
STANFORDCARS_DIR = ""
FLOWERS102_DIR = ""
...
if hparams['dataset'] == 'imagenet':
    ...
    hparams['data_dir'] = IMAGENETV2_DIR
    ...
    # hparams['data_dir'] = pathlib.Path(IMAGENETV2_DIR)
    hparams['data_dir'] = IMAGENETA_DIR
    dataset = RemappedImageFolder(root=hparams['data_dir'], transform=tfms, remapping=swapped_dict)
    # classes_to_load = openai_imagenet_classes
    classes_to_load = None
    hparams['descriptor_fname'] = 'my_imagenet.json'
    # dataset.classes = classes_to_load
    ...
    hparams['data_dir'] = CALTECH101_DIR
    dataset = torchvision.datasets.Caltech101(hparams['data_dir'], transform=tfms)
    classes_to_load = None #dataset.classes
    hparams['descriptor_fname'] = 'my_caltech101.json'
...
######################
def compute_label_encodings_clippy(model):
    label_encodings = model.text.encode([hparams['label_before_text'] + wordify(l) + hparams['label_after_text'] for l in label_to_classname], convert_to_tensor=True)
    return label_encodings
######################
