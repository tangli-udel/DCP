        ...
        for header, d in self.forks.items():
            if name.startswith(header.split('.')[0]+'.'):
                ...
                if next_.isnumeric() and prev_ + '.' + next_ == header:
                    ...
                    # print("number: " + name[len(header)+1:])
                elif next_ == '*':
                    d.register(name[len(prev_ + '.*')+1:], func)
                    # print("*: " + name[len(prev_ + '.*')+1:])
                else:
                    # d.register(name[len(header)+1:], func)
                    # print("non: " + name[len(header)+1:])
                    ###########################
                    slice_index = len(header) + 1 if name[len(header)] == '.' else len(header) + 2
                    d.register(name[slice_index:], func)
                    # print("non: " + name[slice_index:])
                    ###########################
