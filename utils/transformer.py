...
class MultiheadAttention(nn.Module):
    ...
    def forward_direct_hook(self, x, attn_mask=None):
        B, N, C = x.shape
        qkv = self.hook(
            "in_proj_bias.post",
            ret=self.hook("in_proj.post", ret=x @ self.in_proj_weight.T)
    ...
    def forward_per_head_simple(self, x, attn_mask=None):
        self.layer_counter += 1
        ...
        v = torch.einsum("bnc,hdc->bhnd", x, v_weight) + v_bias # (B, self.num_heads, N, self.head_dim)
        dk = q.size()[-1]
        q = q / math.sqrt(dk)
        # q = self.hook("q_norm", ret=q)
        attn = q @ k.transpose(-2, -1)
    ...
    def forward(self, x, attn_mask=None, method: Text = "ov_circuit"):
        ...
        elif method == "direct_hook":
            x = self.forward_direct_hook(x, attn_mask=attn_mask)
        elif method == "qkv":
        ...
        elif method == "head_simple":
            x = self.forward_per_head_simple(x, attn_mask=attn_mask)
        self.hook.finalize()
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attn_method: Text = "direct", dense=False):
        self.output_tokens = dense
